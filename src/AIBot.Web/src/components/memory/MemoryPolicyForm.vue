...
function updateSummaryModel(key: keyof ModelSettings, value: unknown) {
  ...
  const model = structuredClone(next.summaryModel ?? newSummaryModel()) as unknown as Record<string, unknown>
  model[key] = value
...
<template>
  <div class="policy-form">
    <section class="form-section">
      ...
      <div v-for="field in numericFields" :key="field.key" class="policy-row">
    ...
    <section class="form-section">
      ...
      <div v-for="field in booleanFields" :key="field.key" class="policy-row boolean-row">
        <div class="field-copy"><strong>{{ field.label }}</strong><span>{{ field.help }}</span></div>
        <el-switch v-if="isNpc" :model-value="hasOverride(field.key)" inline-prompt active-text="覆盖" inactive-text="继承" @update:model-value="toggleOverride(field.key, Boolean($event))" />
    ...
    <section class="form-section">
      ...
      <div v-if="isNpc" class="policy-row">
      ...
      <div v-else class="policy-row">
        ...
        <span></span><el-switch :model-value="modelValue.summaryModel !== null" @update:model-value="toggleGameSummaryModel(Boolean($event))" /><span class="source-tag">game</span>
      </div>
...
.policy-row { min-height: 68px; padding: 12px 16px; display: grid; grid-template-columns: minmax(260px, 1fr) 86px minmax(160px, 210px) 92px; align-items: center; gap: 12px; border-bottom: 1px solid #edf0f5; }
...
@media (max-width: 1280px) { .policy-row { grid-template-columns: minmax(220px, 1fr) 78px minmax(150px, 180px) 82px; } }
</style>
