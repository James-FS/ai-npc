...
CREATE TABLE IF NOT EXISTS memory_audits (
  ...
  KEY ix_memory_audits_filter (game_id,npc_id,player_id,action,ts)
        ...
        /// <summary>按版本执行的迁移。版本 1 覆盖原有基础表，版本 2 增加可恢复摘要任务。</summary>
        public static readonly Migration[] Migrations =
        {
            new Migration(1, "initial", string.Join(";\n", Statements)),
            new Migration(2, "memory-summary-jobs", @"
